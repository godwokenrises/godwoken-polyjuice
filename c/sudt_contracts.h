...
int transfer_to_any_sudt(gw_context_t* ctx,
  ...
  ret = parse_u128(input_src + 52, &amount);
  if (ret != 0) {
    return ERROR_TRANSFER_TO_ANY_SUDT;
  }
...
/* input:
   ======
   input[ 0..32 ] => sudt_id (big endian)
   input[32..52 ] => owner_id (address)
   input[52..72 ] => spender_id (address)
   input[72..104] => amount (big endian)
...
int set_allowance(gw_context_t* ctx,
  ...
  ret = parse_u128(input_src + 72, &amount);
  if (ret != 0) {
    return ERROR_SET_ALLOWANCE;
  }
  ...
  evmc_address owner_address = *((evmc_address *)input_src + 32);
  evmc_address spender_address = *((evmc_address *)input_src + 52);
  ...
  ret = sudt_set_allowance(ctx, sudt_id, owner_id, spender_id, amount);
  if (ret != 0) {
