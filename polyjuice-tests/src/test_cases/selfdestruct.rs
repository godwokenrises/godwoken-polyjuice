...
use crate::helper::{
    new_block_info,
    new_account_script,
    account_id_to_eth_address,
    setup,
    PolyjuiceArgsBuilder,
    CKB_SUDT_ACCOUNT_ID,
};
...
fn test_selfdestruct() {
    ...
    assert_eq!(tree.get_sudt_balance(CKB_SUDT_ACCOUNT_ID, beneficiary_id).unwrap(), 0);
    ...
    let new_account_id = tree.get_account_id_by_script_hash(&contract_account_script.hash().into()).unwrap().unwrap();
    assert_eq!(tree.get_sudt_balance(CKB_SUDT_ACCOUNT_ID, new_account_id).unwrap(), 200);
    assert_eq!(tree.get_sudt_balance(CKB_SUDT_ACCOUNT_ID, beneficiary_id).unwrap(), 0);
    {
    ...
    assert_eq!(tree.get_sudt_balance(CKB_SUDT_ACCOUNT_ID, new_account_id).unwrap(), 0);
    assert_eq!(tree.get_sudt_balance(CKB_SUDT_ACCOUNT_ID, beneficiary_id).unwrap(), 200);
}
